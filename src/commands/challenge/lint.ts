...
export default class LintCommand extends Command {
    ...
    async run(): Promise<void> {
        ...
        const ignoredCodes = flags.ignore.split(',').map(c => c.trim().toUpperCase());
        ...
        const ignoredFindings = failedFindings.filter(f => ignoredCodes.includes(f.rule.code)),
            actionableFindings = failedFindings.filter(f => !ignoredCodes.includes(f.rule.code));

        const groupedFindings = this.partitionFindingsByLevel(actionableFindings);
        ...
        if(flags.format === 'text') {
            ...
            this.reportFindingsText(groupedFindings.fatal, '⛔ ', chalk.bgRed.whiteBright);
            this.reportFindingsText(groupedFindings.error, ' ❗', chalk.red);
            this.reportFindingsText(groupedFindings.warning, '⚠️ ', chalk.yellow);
    ...
    private partitionFindingsByLevel(findingList: LintResult[]): Record<RuleLevel, LintResult[]> {
    ...
    private reportFindingsText(findingList: LintResult[], prefix = '', color: chalk.Chalk = chalk.reset): void {
        prefix = process.stdout.isTTY ? `${prefix} ` : '';
        findingList.forEach(finding => {
            this.log(color(`${prefix}${finding.rule.level} ${finding.rule.code}: ${finding.rule.description}`));
            if(finding.violation?.message) {
                this.log(color(`${' '.repeat(prefix.length+2)}  ${finding.violation.message}`));
            }
        });
    }

    private reportFindingsJson(findingsSummary: FindingSummary): void {
        this.log(JSON.stringify(findingsSummary, null, process.stdout.isTTY ? 2 : undefined));
    }
}