...
export default class TemplateGenerator {
    ...
    async generate(): Promise<void> {
        if (await fs.pathExists(this.targetDir)) {
            throw new Error(
                `"${this.targetDir}" already exists, refusing to initialize challenge.`
            );
        }
        ...
        await Promise.all(
            (
                await ignoreWalk(tmpDir)
            ).map(async (file) => {
                const stat = await fs.lstat(file);
                if (stat.isFile() && file.endsWith(TEMPLATE_FILE_EXTENSION)) {
                    log.debug("Processing template file", file);

                    const resultantFilePath = file.slice(
                        0,
                        -TEMPLATE_FILE_EXTENSION.length
                    );
                    await fs.writeFile(
                        resultantFilePath,
                        await templateFile(file, this.challengeContext)
                    );
