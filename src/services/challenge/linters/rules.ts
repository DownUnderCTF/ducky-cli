...
export const rules: Readonly<Readonly<RuleDefinition>[]> = [
    // Structure rules
    {code: 'S000', check: S000, level: 'fatal',   description: 'Must have a ctfcli.yml file'},
    {code: 'S001', check: S001, level: 'error',   description: 'Must have a solve/ folder'},
    {code: 'S002', check: S002, level: 'error',   description: 'Must have a solve/WRITEUP.md file with content'},
    {code: 'S003', check: S003, level: 'error',   description: 'Must have a README.md file with content'},
    {code: 'S004', check: S004, level: 'error',   description: 'Must not have DELETEME files'},

    // Configuration rules
    {code: 'C000', check: C000, level: 'error',   description: 'Must have a challenge name'},
    {code: 'C001', check: C001, level: 'error',   description: 'Must have a valid challenge category'},
    {code: 'C002', check: C002, level: 'error',   description: 'Must have a challenge description'},
    {code: 'C003', check: C003, level: 'warning', description: 'Should have a challenge author (specified by "Author: ...")'},
    {code: 'C004', check: C004, level: 'error',   description: 'Must have at least a difficulty tag'},
    {code: 'C005', check: C005, level: 'error',   description: 'Must have at least one flag'},
    {code: 'C006', check: C006, level: 'error',   description: 'Published paths must exist'},
];
...
async function C001(challengeDir: string): Promise<string | null> {
    ...
    if(!CHALLENGE_CATEGORIES.includes(ctfcliSpec.category)) {
        return `Invalid category ${ctfcliSpec.category}. Must be one of ${CHALLENGE_CATEGORIES.join(', ')}`;
    }
...
async function C006(challengeDir: string): Promise<string | null> {
    ...
    const fileStatuses = await Promise.all(
        requiredFiles.map(async p => [p, await fs.pathExists(path.join(challengeDir, p))]),
    );
