...
export default class Linter {
    ...
    public async lint(): Promise<LintResult[]> {
        ...
        const fatalViolations = await this.processRuleSet(rules.filter(r => r.level === 'fatal'));
        if(fatalViolations.filter(r => r.violation !== null).length > 0) {
            return fatalViolations;
        }

        // 2. Process everything else
        return await this.processRuleSet(rules.filter(r => r.level !== 'fatal'));
    }

    private async processRuleSet(rules: RuleDefinition[]): Promise<LintResult[]> {
    ...
    private async processRule(rule: RuleDefinition): Promise<RuleViolation | null> {
